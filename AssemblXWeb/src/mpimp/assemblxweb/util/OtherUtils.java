...
public class OtherUtils {
	...
	public static int calculateLeadingDigits(Integer counter) {
	...
	public static String createMasterPlasmidEntryName(AssemblXWebModel model, Integer plasmidNumber) {
		String initialPlasmidName = "p";
		int loginNameLength = model.getOperator().getLogin().length();
		if (loginNameLength >= 3) {
			initialPlasmidName += model.getOperator().getLogin().substring(0, 3);
		} else {
			initialPlasmidName += model.getOperator().getLogin().substring(0, loginNameLength);
		}
		int noOfZeros = 4 - OtherUtils.calculateLeadingDigits(plasmidNumber);
		for (int i = 0; i < noOfZeros; i++) {
			initialPlasmidName += "0";
		}
		initialPlasmidName += plasmidNumber;
		return initialPlasmidName;
	}
}
